<template>
	<view class="content">
		...
		<view class="content-box">
			...
			<button class="btn" :style="btn?'':'background:#bbbbbb'" :disabled="!btn" @click="login()">登 录</button>
...
export default {
	...
	methods:{
		...
		login(){
			if(this.testPhone()&&this.testPassword()){
				var data={
					...
					login(data).then((data)=>{
						if(data.data.msg="登录成功"){
							...
							this.changToken(data.data.data.token)
							uni.navigateBack({
								delta:1
							})
						}
		...
		qq(){
			...
			var url = "/api/menu/user"
			var data={
				
			}
			Get.urlRequset(url,data).then((data)=>{
				console.log(data)
			})
